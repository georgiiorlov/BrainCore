...
def get_phonenumber(phone: str) -> str:
    ...
    if not re.match(PHONE_PATTERN, phone):
        raise ValueError("Номер должен содержать 10-15 цифр и может начинаться с '+'")
    return phone
...
def get_age(age: str) -> int:
    ...
    if age < 14 or age > 120:
        raise ValueError("Возраст должен быть в диапазоне 14-120")
    return age
...
def get_password(password: str) -> str:
    ...
    if not re.fullmatch(PASSWORD_PATTERN, password):
        raise ValueError(
            "Пароль должен быть 8-64 символа и содержать буквы и цифры"
        )
